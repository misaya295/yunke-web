`<template>
  <div class="app-container">
    <div class="filter-container">
       <!-- 报销名称 -->
      <el-input v-model="funding.name" placeholder="请输入报销名称" style="width:155px;" class="filter-item search-item"></el-input>

      <!-- 状态 -->
      <el-select v-model="funding.state" placeholder="状态" class="filter-item search-item" style="width:100px">
        <el-option v-for="item in statusTable" :key="item.value" :label="item.label" :value="item.value">
        </el-option>
      </el-select>
      <!-- 申请人 -->
      <el-autocomplete popper-class="my-autocomplete" v-model="funding.proposerName" :fetch-suggestions="querySearch" placeholder="请选择申请人" @select="handleSelect1" style="width: 155px" class="filter-item search-item">
    ...
    <div class="filter-item">
      ...
      <el-button v-hasPermission="['funding:add']" class="filter-item" type="success" plain icon="el-icon-plus" @click="handleCreate">
        添加
      </el-button>
      ...
      <!-- 查询结果 -->
      <el-table ref="table" :data="fundingTableData" style="width:100%" border fit @selection-change="onSelectChange" @row-click="toogleExpand" :row-key="getRowKey">
        ...
        <el-table-column label="状态" :filters="[{ text: '申请中', value: '1' }, { text: '报销中', value: '2' },{ text: '报销成功', value: '3' },{ text: '申请失败', value: '4' }]" :filter-method="filterBXStatus" align="center" min-width="100px">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('table.operation')" align="center" min-width="105px" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-tooltip
              ...
              :enterable="false"
            >
            <i v-hasPermission="['funding:view']" class="el-icon-info table-operation"
            style="color: #909399;" @click.stop="toogleExpand(scope.row)" />
            </el-tooltip>
          ...
          <el-tooltip
              ...
              :enterable="false"
            >
            <i v-hasPermission="['funding:update']" class="el-icon-setting table-operation" style="color: #2db7f5;" @click="handleUpdate(scope.row)" />
          </el-tooltip>
            <el-tooltip
              ...
              :enterable="false"
            >
            <i v-hasPermission="['funding:delete']" class="el-icon-delete table-operation" style="color: #f50;" @click="handleDelete(scope.$index, scope.row)" />
            </el-tooltip>
            <el-tooltip
        ...
        <template slot-scope="{row}">
          ...
          >
            <el-row>
              <el-col :span="8">
                <el-form-item class="view-item">
                  ...
                  <span><i class="el-icon-bank-card" /> {{ '银行卡号：' }}</span>
                  {{ row.card }}
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item class="view-item">
                  ...
                  <span><i class="el-icon-time" /> {{ '申请时间：' }}</span>
                  {{ row.applyTime }}
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item class="view-item">
                  ...
                  <span><i class="el-icon-time" /> {{ '成功报销时间：' }}</span>
                  {{ row.successTime }}
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item class="view-item">
                  <span><i class="el-icon-printer" /> {{ '任务：' }}</span>
                  {{ getTaskName(row) }} 
                  <el-tooltip
                    ...
                    :enterable="false"
                  >
                  <i class="el-icon-share table-operation" v-if="row.taskId!==null&&row.taskId!==''&&judgeTypeIsTask(row.type)" @click="jump(row)"></i>
                  </el-tooltip>
                ...
                <el-col :span="24">
                  <el-form-item class="view-item">
                    <div class="demo-image">
                      <span><i class="el-icon-s-ticket" />  {{ '发票：' }}</span>
                      <div class="block" v-for="(item, i) in (row.invoice===null||row.invoice==='')?[]:row.invoice.split(',')" :key="i" >
                        <el-image
                          :src="item"
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :width="width"
        ...
        <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="70px" top="3vh">
          ...
          <el-form-item label="证明人">
            <el-autocomplete popper-class="my-autocomplete" v-model="temp.certifierName" :fetch-suggestions="querySearch2" placeholder="请选择证明人" @select="handleSelect3" style="width: 100%" class="filter-item">
              <template slot-scope="{ item }">
                <div class="name">{{ item.fullName }} <span class="id">{{ item.userId }}</span></div>
              </template>
            </el-autocomplete>
      ...
      <el-dialog :title="temp.state | ViewTextFilter" :visible.sync="fundingViewVisible" :width="width" :close-on-click-modal="false" :close-on-press-escape="false">
        <el-row :gutter="10">
          <el-col :xs="24" :sm="12">
            <div class="view-item">
      ...
      <el-dialog title="导入报销" :visible.sync="fundingImportVisible" :width="width" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <!-- 图片预览 -->
    <el-dialog title="图片预览" :visible.sync="previewVisible" width="40%" @close="previewDialogClose" :close-on-click-modal="false" :close-on-press-escape="false">
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        for (let j = 0; j < this.invoiceFiles.length; j++) {
          if (this.invoiceFiles[j].uid === file.uid) {
            ...
            if (this.invoiceFiles.length === 1) {
              ...
              if (j === 0) {
                this.temp.invoice = this.temp.invoice.split(
                  fileUrl + ','
                )[1]
              } else {
                this.temp.invoice = this.temp.invoice.split(
      ...
      handleCheck(row) {
         if (row.state !== 1) { //fundingIds等于"", 说明没有选择状态为失败的报销
          this.$message({
      ...
      judgeTypeIsTask(type){
        if(type==="thesis"||type==="match"||type==="items"||type==="copyright") return true;
        return false;
      },
      getStaskInFo(id) {
      ...
      setTemp(r) {
        ...
        //r有值就用dealId转化一下 
        if(r.data) r = this.dealId(r.data.data); //if(r.data)处理的是接口返回的r
        else if(r) r = this.dealId(r);        //else if(r)处理的是其他条件的r

        if(r.type && reg.test(r.type)) {this.taskType = r.type;}
        // else this.taskType = "";
